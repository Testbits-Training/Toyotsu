...
def static "util.copyToExcel.exelInboundPlan2"(
    	File file	
     , 	String name	
     , 	int rowNum	
     , 	int colNum	) {
    (new util.copyToExcel()).exelInboundPlan2(
        	file
         , 	name
         , 	rowNum
         , 	colNum)
...
def static "util.handlePicker.handleCalendar"(
    	TestObject currentMonthYear	
     , 	TestObject nextMonthButton	
     , 	int inputDay	
     , 	int inputMonth	) {
    (new util.handlePicker()).handleCalendar(
        	currentMonthYear
         , 	nextMonthButton
         , 	inputDay
         , 	inputMonth)
...
def static "util.testing.otpKey"(
    	String name	) {
    (new util.testing()).otpKey(
        	name)
}


def static "util.generateTime.currentMonth"() {
    (new util.generateTime()).currentMonth()
}


def static "util.generateTime.currentDateDay"() {
    (new util.generateTime()).currentDateDay()
}
